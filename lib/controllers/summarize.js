...
function clean(tokens, string) {
  for (var i = 0; i < tokens.length; i++) {
    if (tokens[i].toLowerCase() == string || tokens[i].length < 3) {
      tokens.splice(i, 1);
      i--;
    }
  }
...
function tokenizeSentence(sentence, callback) {
  ...
  if(sentence.indexOf('.') === -1) {
	...
	tokens = (sentence.match(/([0-9]*?\.[0-9]*?)|([^\s\.,!?]+)/g) || []);
  tokens = clean(tokens, "");
  callback(tokens);
}
  ...
  for( var i = 0; i < sentenceTokens.length; i++ ) {
    ...
    for( var j = 0; j < sentenceTokens.length; j++ ) {
      if( j !== i ) {
        ...
        var length = sentenceTokens[i].length;

        if( length < 5 ) {
          length = 6;
        }
        sentenceGraph[i][j] = wordsCount/((length + sentenceTokens[j].length)/2);
      }
      else {
        sentenceGraph[i][j] = 0;
      }
