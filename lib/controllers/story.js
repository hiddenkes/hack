...
exports.list = function(req, res) {
  ...
  var interestRef = myRootRef.child(interest);
  interestRef.on('value', function(snapshot) {
    var val = snapshot.val();
    var lastDate = (new Date()).getTime() - 1800000;
    if(val === null || !val.cache || val.cached > lastDate) {
      request('http://api.feedzilla.com/v1/articles/search.json?count=50&q=' + interest, function(error, response, body){
        if (!error && response.statusCode == 200) {
          ...
          _.each(info.articles, function(article){
            ...
            todo.push({p: p, url: article.url.substring(0, article.url.indexOf('?'))});
          });

          async.mapLimit(todo, 4, doreq, function(err, results){
            ...
            for(var i = 0; i < results.length; i++){
              for (var j in results[i]) {
                if (results[i][j] === null || results[i][j] === undefined) {
                  delete results[i][j];
                }
              }
            }
            interestRef.update({'cache': results, 'cached': (new Date()).getTime()});
          });
        }else{
          res.json({ok: false});
        }
