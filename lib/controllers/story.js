      ...
      request('http://api.feedzilla.com/v1/articles/search.json?count=30&q=' + interest, function(error, response, body){
        if (!error && response.statusCode == 200) {
          ...
          async.mapLimit(todo, 2, doreq, function(err, results){
            ...
            try {
              interestRef.update({'cache': lodash.cloneDeep(results), 'cached': (new Date()).getTime()});
            }catch(e){
              //TODO: ERROR HANDLE:
              console.log(e);
            }
...
function doreq(obj, crb){
  ...
  request(uri, function(error, response, body){
    ...
    if(url){
      ...
      url = url.replace('news.feedzilla.com', '109.123.81.101');
      //console.log(url);
